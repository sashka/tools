...
then
    ...
    echo "for example, static file: `basename $0` http://fe.banners.mail.ru/js/show.js cookie.txt"
    echo "or dynamic file: `basename $0` http://fe.banners.mail.ru/js/1000.js cookie.txt"
...
if [[ "x$2" = "xNO" ]]
then
    CURL_DATA=`curl -o /dev/null -w '%{time_total} %{time_connect}\n' $URL 2>/dev/null`
else
    CURL_DATA=`curl -b cookie.txt -o /dev/null -w '%{time_total} %{time_connect}\n' $URL 2>/dev/null`
fi
if [[ "x$CURL_DATA" = "x" ]]
then
